...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    active_device_label = models.CharField(max_length=200, blank=True, default='', help_text='Browser/device of the current active session.')
    active_login_at = models.DateTimeField(null=True, blank=True, help_text='When the current active session was created.')
...
class DailyUpdatePost(models.Model):
    ...
    NEWS_INTERNATIONAL = 'International'
    NEWS_STATE = 'State'
    NEWS_SPORTS = 'Sports'
    ...
    news_category = models.CharField(
        max_length=20, choices=NEWS_CATEGORY_CHOICES, blank=True,
        help_text='Used for Current Affairs only — groups the post under this heading for its date.',
    )
    ...
    def video_url_is_direct_file(self):
        return bool(self.video_url) and self.video_url.lower().split('?')[0].endswith(('.mp4', '.webm', '.ogg'))
...
class Course(models.Model):
    ...
    enable_folders = models.BooleanField(default=False, help_text='Organize this course\'s videos or PDFs inside folders and subfolders.')
    ...
    thumbnail = models.ImageField(upload_to='courses/', max_length=500, blank=True, null=True, help_text='Recommended size: 400×240px (16:10).')
    pdf_file = models.FileField(upload_to='elibrary_pdfs/', max_length=500, blank=True, null=True, help_text='Used for E-Library only. Upload the book/notes as a PDF.')
    video_file = models.FileField(upload_to='course_videos/', max_length=500, blank=True, null=True, help_text='Used for Video Courses only. Upload the course video (MP4 recommended).')
    ...
    max_optional_sections = models.PositiveIntegerField(
        default=1,
    ...
    def highlight_list(self):
        return [line.strip().lstrip('-').strip() for line in self.highlights.splitlines() if line.strip().lstrip('-').strip()]

    def __str__(self):
        return self.name


class CourseContentFolder(models.Model):
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name='content_folders')
    ...
    def clean(self):
        super().clean()
        if self.course_id and self.course.course_type not in (Course.VIDEO_COURSE, Course.ELIBRARY):
            raise ValidationError({'course': 'Content folders are only available for Video Courses and E-Library items.'})
        if not self.parent_id:
            return
        if self.parent_id == self.pk:
            raise ValidationError({'parent': 'A folder cannot be its own parent.'})
        if self.parent.course_id != self.course_id:
            raise ValidationError({'parent': 'The parent folder must belong to the same course.'})

        ancestor = self.parent
        visited = set()
        while ancestor:
            if ancestor.pk == self.pk:
                raise ValidationError({'parent': 'A folder cannot be moved inside one of its subfolders.'})
            if ancestor.pk in visited:
                raise ValidationError({'parent': 'The selected folder hierarchy contains a cycle.'})
...
class CourseVideo(models.Model):
    ...
    folder = models.ForeignKey(CourseContentFolder, on_delete=models.SET_NULL, null=True, blank=True, related_name='videos')
    title = models.CharField(max_length=200)
    video_file = models.FileField(upload_to='course_videos/playlist/', max_length=500, help_text='Upload an MP4 or other browser-supported video file.')
    ...
    def clean(self):
        super().clean()
        if self.folder_id and self.folder.course_id != self.course_id:
            raise ValidationError({'folder': 'The folder must belong to this video course.'})
...
class CourseDocument(models.Model):
    ...
    folder = models.ForeignKey(CourseContentFolder, on_delete=models.SET_NULL, null=True, blank=True, related_name='documents')
    title = models.CharField(max_length=200)
    pdf_file = models.FileField(upload_to='elibrary_pdfs/collection/', max_length=500, help_text='Upload a PDF document.')
    ...
    def clean(self):
        super().clean()
        if self.folder_id and self.folder.course_id != self.course_id:
            raise ValidationError({'folder': 'The folder must belong to this E-Library item.'})
...
class CourseEnrollment(models.Model):
    ...
    paid_at = models.DateTimeField(null=True, blank=True, help_text='When a paid course payment was confirmed.')
...
class StoreOrder(models.Model):
    ...
    razorpay_payment_id = models.CharField(max_length=100, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    paid_at = models.DateTimeField(null=True, blank=True, help_text='When payment for this order was confirmed.')

    class Meta:
        ordering = ['-created_at']

    def save(self, *args, **kwargs):
        paid_statuses = {self.STATUS_PAID, self.STATUS_SHIPPED, self.STATUS_DELIVERED}
        if self.status in paid_statuses and self.paid_at is None:
...
class ExamTickerSettings(models.Model):
    heading = models.CharField(
        max_length=180,
        default="India's Most Competitive Exams in One Platform",
    )
    animation_duration = models.PositiveIntegerField(
        default=20,
        help_text='Seconds for one full ticker loop. A lower number moves faster.',
    )
...
class ExamTickerItem(models.Model):
    ...
    icon = models.CharField(
        max_length=20,
        blank=True,
        help_text='Emoji or a short symbol shown before the label.',
    )
    link = models.CharField(
        max_length=500,
        blank=True,
        help_text='Optional website path, page anchor, or full URL.',
    )
    order = models.PositiveIntegerField(default=0)
    is_active = models.BooleanField(default=True)
...
class Bundle(models.Model):
    ...
    image = models.ImageField(
        upload_to='bundles/', max_length=500, blank=True, null=True,
        help_text='Optional. Replaces the emoji on bundle cards. Recommended size: 800×450px.',
    )
    icon = models.CharField(max_length=10, blank=True, default='🎓', help_text='Emoji shown on the bundle card, e.g. 🎓')
    badge_label = models.CharField(max_length=150, blank=True, help_text='e.g. All Subjects Included. For multiple small tags (like subjects), separate with commas, e.g. Mathematics, English, GK')
    students_label = models.CharField(max_length=30, blank=True, default='10,000+', help_text='Shown next to the students icon on the card, e.g. 21,230')
    access_label = models.CharField(max_length=30, blank=True, default='Full Course', help_text='Shown next to the clock icon on the card, e.g. Lifetime Access, Full Course')
    original_price = models.DecimalField(max_digits=8, decimal_places=2, default=0)
    current_price = models.DecimalField(max_digits=8, decimal_places=2, default=0)
    ...
    @property
    def badge_tags(self):
        return [tag.strip() for tag in self.badge_label.split(',') if tag.strip()]
...
class QuizGameSettings(models.Model):
...
class StaffSalaryPayment(models.Model):
    ...
    def __str__(self):
        return f'{self.staff.name} — {self.month:%b %Y}'
